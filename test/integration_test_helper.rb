...
class ActionDispatch::IntegrationTest
  ...
  def create_event(attrs={})
    ...
    with_options scope: "activerecord.attributes.event" do |label|
      ...
      fill_in "event[start_at][date]", with: 2.hours.from_now.strftime("%Y-%m-%d")
      fill_in "event[start_at][time]", with: 2.hours.from_now.strftime("%H-%M")
      fill_in "event[end_at][date]",   with: attrs.fetch(:start_at).strftime("%Y-%m-%d")
